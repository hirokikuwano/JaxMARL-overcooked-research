...
def main(config):
    ...
    # vmap出力なので先頭seed (index 0) を取り出す
    runner_state = out["runner_state"]
    params = jax.tree_util.tree_map(lambda x: x[0], runner_state[0].params)

    save_path = os.path.join(
        save_dir, f"params_{layout_name}_shared_{ts}.msgpack"
    )
    with open(save_path, "wb") as f:
        f.write(to_bytes(params))
    print(f"[Shared] Saved params -> {save_path}")
